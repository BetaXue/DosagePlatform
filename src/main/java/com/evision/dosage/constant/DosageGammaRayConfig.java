 ...
 **/
@Component
public class DosageGammaRayConfig {
    @Autowired
    ...
    public void initData() {
        ...
        //网格N*N
        map.put("cellCount", DosageConstant.GAMMA_RAY_CELL_COUNT);
        List list = new ArrayList();
        List<GammaRay> gammaRays = gammaRayService.findAllGroupByGridNumber();
        for (GammaRay gammaRay : gammaRays) {
            String gridNumber = gammaRay.getGridNumber();
            List<GammaRay> objectList = new ArrayList();
            List<GammaRay> encryGrids = gammaRayService.findByGridNumber(gridNumber);
            objectList.addAll(encryGrids);
            int[] ints = this.gridNumber2Coordinate(gridNumber);
            if (objectList.size()>1){
                // 再次计算平均值
                BigDecimal avg = BigDecimal.ZERO;
                for (GammaRay ray : objectList) {
                    avg = BigDecimalUtils.safeAdd(avg,new BigDecimal(ray.getAverageValue()));
                }
                BigDecimal result = BigDecimalUtils.safeDivide(avg,objectList.size());
                for (GammaRay ray : objectList) {
                    ray.setAvgValue(result.toString());
                }
            }
            Object[] obj = new Object[]{ints[0], ints[1], objectList};
            list.add(obj);
    ...
    private static int[] gridNumber2Coordinate(String gridNumber) {
        int[] cellCount = DosageConstant.GAMMA_RAY_CELL_COUNT;
        //行数35
