...
@Slf4j
@Service
@DosageServiceConfig("food")
public class FoodRadionuclideServiceImpl extends ServiceImpl<FoodWaterRadionuclideMapper, FoodWaterRadionuclideEntity> implements FoodWaterRadionuclideService {
    @Autowired
    ...
    public boolean dealData(List<FoodWaterRadionuclideEntity> list) throws Exception {
        ...
        if (!CollectionUtils.isEmpty(list)) {
            ...
            for (FoodWaterRadionuclideEntity foodWaterRadionuclideEntity : list) {
                ...
                query.eq("type", 0).eq("sample_number", sampleNumber).eq("disabled", 1).eq("deleted", 0);
                ...
                if (CollectionUtils.isEmpty(existsEntityList)) {
                    ...
                    foodWaterRadionuclideEntity.setType(0);
    ...
    public List<FoodWaterRadionuclideEntity> importExcel(MultipartFile multipartFile) throws Exception {
        ...
        for (int sheetIndex = 0; sheetIndex < sheetCount; sheetIndex += 1) {
            ...
            //食材中放射性核素活度浓度
            if (DosageExcelEnum.FOOD_RADIONUCLIDE.getExcelName().equals(fileName) ||
                    DosageExcelEnum.FOOD_RADIONUCLIDE.getExcelName().equals(sheetName)) {
                //检查行名
    ...
    @Override
    public List<FoodWaterRadionuclideEntity> getDataList(String queryName, String sortName, Integer isAsc, Integer incloudDe) {
        QueryWrapper<FoodWaterRadionuclideEntity> query = new QueryWrapper<FoodWaterRadionuclideEntity>();
        query.eq("type", 0).eq("disabled", 1);
        if (incloudDe == 0) {
    ...
    public List<FoodWaterRadionuclideEntity> getHistory(int parameter) throws Exception {
        FoodWaterRadionuclideEntity radionuclideEntity = foodWaterRadionuclideMapper.selectById(parameter);
        QueryWrapper<FoodWaterRadionuclideEntity> query = new QueryWrapper<FoodWaterRadionuclideEntity>();
        query.eq("sample_number", radionuclideEntity.getSampleNumber())
                .eq("disabled", 0)
                .eq("deleted", 0)
                .eq("type", 0)
