...
public class BigDecimalUtils {
    ...
    public static String convertNum(String num, Integer scale) {
        if (num == null || "".equals(num)) {
            return null;
        }
        return df2.format(new BigDecimal(num).setScale(scale, BigDecimal.ROUND_HALF_UP)).toString();
    }

    public static String convertNumRatio(String num) {
        if (num == null || "".equals(num)) {
            return null;
        }
        return new BigDecimal(num).multiply(BigDecimal.valueOf(100)).setScale(4, BigDecimal.ROUND_HALF_UP).toString();
    }
}
